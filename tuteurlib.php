...
function report_tuteur_filterActivities($tabActivities) {
    ...
    foreach ( $tabActivities as $activity ) {
        if (stripos($type_act, "&" . $activity->modname . "&") !== false) { //id 21
            $ret[] = $activity;
        }
    }
    return $ret;
...
function report_tuteur_createActivitiesFilter() {
    ...
    if ($paramGroup == null) {
    	$filterTypeActivities = $filterTypeActivities . "assign&quiz&journal&lesson&" ;
    } else {
...
function report_tuteur_StudentList($course) {
	...
	$userlist = $DB->get_recordset_sql ( $sql, array (
			$course->id 
	) );
	$data = array ();
...
function report_tuteur_getModifAssignGrades($idUser, $idActivity) {
	global $DB;
	$cours_mod = $DB->get_record ( "course_modules", array (
			'id' => $idActivity 
	) );
	$rs = $DB->get_field ( 'assign_grades', 'MAX(timemodified)', array (
			'assignment' => $cours_mod->instance,
			'userid' => $idUser 
	) );
	if (isset ( $rs )) {
		return $rs;
...
function report_tuteur_JournalState($idUser, $idActivity) {
	...
	$inJournal = $DB->get_record_select ( 'journal_entries', 'journal = ? and userid = ?', array (
			$infoActivity->instance,
			$idUser 
	) );
...
function report_tuteur_getContext($idActivity) {
	global $DB;
	$rs = $DB->get_field_sql ( 'SELECT id FROM {context} WHERE instanceid = ? AND contextlevel = 70', array (
			$idActivity 
	) );
	return $rs;
}
